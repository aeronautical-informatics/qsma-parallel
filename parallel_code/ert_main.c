...
#define RTT_TIME	10			/* Round-trip time for one timing loop. */
#define MAXSZ		1024			/* Maximum message size. */
#define MAXLINELEN	1024         /* Maximum amount of data we're going to write. */
...
void print_flight_data(const struct FlightData *data)
{
    printf("\n Flight parameters\n PITCH: %.3f\t HEIGHT: %.3f\t CLIMB_RATE: %d\n", data->pitch, data->height, data->climb_rate);
    printf("\n longitude: %.5f\t \n", data->longitude);
    printf("\n Latitude: %.5f\t \n", data->latitude);
    printf("\n groundspeed: %.5f\t \n", data->groundspeed);
    printf("\n vertical-speed: %.5f\t \n", data->vertical_speed);
...
static void *thread_r(void *__unused)
{
	char buf[MAXSZ];
	int r;
...
/*Main for streaming data from internal partition*/
//int main(void)
//{
//	pthread_attr_t attr;
//	pthread_t tid_w;
...
//	mattr.mq_flags = 0;  // Ensure blocking mode
...
//	while (1)
//	{
//		struct FlightData fdmData;
//		// Read messages from mq_in and sedn them back on mq_out.
//		if ((size = mq_receive(mq_in, buf, sizeof(buf), 0)) != -1)
//		{
//			sscanf(buf, "%f %f %d %f %f %f %f %f %f \n",&fdmData.pitch, &fdmData.height,&fdmData.climb_rate, &fdmData.longitude,
//					&fdmData.latitude, &fdmData.groundspeed, &fdmData.vertical_speed, &fdmData.true_track, &fdmData.yaw_rate);
//			print_flight_data(&fdmData);
//			//Initialize the FLTA DATA
//			FLTA_DATA.position.alt=fdmData.height/3.281; //in meters
//			FLTA_DATA.position.lon=fdmData.longitude; //X;
//			FLTA_DATA.position.lat=fdmData.latitude; //Y;
//			FLTA_DATA.Vground=fdmData.groundspeed;
//			FLTA_DATA.VerticalSpeed=fdmData.vertical_speed;
//			FLTA_DATA.RTC=500 * 0.3048; //clearance required in meters
//			FLTA_DATA.TrueTrack=fdmData.true_track; //degrees
//			FLTA_DATA.YawRate=fdmData.yaw_rate; //degrees
//
//			fltastep();
//
//			fflush(stdout);
...
//
//	if (mattr.mq_msgsize != (long)sb.st_blksize) {
//		printf( "Bouncer: Port block sizes don't match.\n");
//		exit(EXIT_FAILURE);
//	}
//
//	/* Set up output message queue. */
